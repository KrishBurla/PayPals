...
export function Sidebar() {
    ...
    <>
      <div className="flex h-[88px] shrink-0 items-center justify-between border-b border-slate-100 px-6">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-teal-600 bg-teal-500 shadow-sm">
  ...
  return (
    <>
      ...
      {/* --- DESKTOP SIDEBAR (Always Fixed, Never Translates) --- */}
      <aside className="flex fixed inset-y-0 left-0 z-50 w-72 flex-col border-r border-slate-200 bg-white">
        {SidebarContent}
      </aside>
      ...
      <aside 
        className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-slate-200 bg-white transition-transform duration-300 ease-in-out lg:hidden ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {SidebarContent}
      </aside>
    </>
